...
# $Id: //depot/asn/lib/Convert/ASN1/_decode.pm#2 $
...
sub _decode {
  ...
  foreach my $buf ($_[0]) {
	...
	TAGLOOP: {
	  my($tag,$len,$npos) = _decode_tl($buf,$pos,$end)
	    or do {
	  ...
	  if ($tag ne $op->[cTAG]) {
	    next OP if $idx >= 0 || defined $op->[cOPT];
	    die "decode error";
	  }
...
# $optn, $op, $stash, $var, $buf, $pos, $len
  ...
  $_[3] = $_[0]->{'decode_timezone'} ? [$time,$offset] : $time;
}
